...
- **A first step on `docs/GOING-LIVE.md`'s Stage 0** (`parallax-cli::run_multi_regime_distribution`): the demo scenario run 200 times per regime, with the ensemble forecast, both venues' quoted prices, and queue position perturbed per seed, independently against three distinct market conditions — `Regime::Baseline`, `Regime::VolatilityShock`, and `Regime::QuietPeriod` — reporting mean/median/p10/p90 net PnL per regime instead of one number, printed by `parallax-demo` alongside the single deterministic run. This is the doc's actual gate, checked rather than described: "p10 is positive... across periods that include at least one volatility shock and one quiet week... one good regime and one bad one is a fail, not an average." Run honestly, it fails: baseline and volatility-shock both clear it cleanly (200/200 seeds profitable in each, p10 of $7.46 and $5.78 respectively), but quiet-period does not — 0/200 profitable, p10 of -$3.36, median -$0.31. Most quiet-period seeds propose orders against a book with no real mispricing in it and finish flat (net PnL exactly 0); the rare seed that does fill loses to fees plus an unfavorable print, not to a modeling bug (traced by hand against individual seeds — a representative losing seed shows 3 fills, 6 orders correctly rejected by the risk gate, and a closing position bought with no edge behind it). That's `MultiRegimeReport::gate_passes()` reporting `FAIL`, which is the point: this strategy configuration has no demonstrated edge in a low-signal regime once fees are charged, and this repo reports that rather than averaging it away or tuning the regime until it passes. Still synthetic data with synthetic noise, not the recorded real venue book data Stage 0 ultimately calls for. Execution latency is deliberately left unperturbed here — see the module doc comment in `edge_distribution.rs` for the idempotency gap that surfaced when an earlier version tried, which is a real finding in its own right.
...
- **Stage 1's clock discipline**, the doc's own line: "signed requests carry timestamps, and skew shows up as authentication failures at the worst possible moment. NTP, monitored, with an alert on drift." This repo has no NTP client (OS-level, out of scope for an application crate) and no live venue credentials yet to actually authenticate with, so what's built is the verifiable piece: `KalshiAdapter::fetch_server_time`/`PolymarketAdapter::fetch_server_time` read each venue's real HTTP `Date` response header — present on every response regardless of the endpoint's own business meaning, so it costs nothing extra to check — and `parallax_venues::ClockSkewMonitor` alerts once *consecutive* readings against the same venue drift past a configurable tolerance (2s default), the same "streak, not a single blip" design `FeedHealthMonitor` and `FeeVerifier` already use elsewhere, since the `Date` header's one-second resolution and lack of round-trip correction make one skewed reading indistinguishable from ordinary network jitter. Wired into `parallax-record`'s existing polling loop alongside feed-health monitoring — every tick now also checks this process's clock against both venues', with configurable thresholds (`PARALLAX_RECORD_MAX_SKEW_MS`, `PARALLAX_RECORD_SKEW_ALERT_AFTER`).
