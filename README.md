...
This repo is the reference implementation of that design. It's a working skeleton with real, tested logic in every layer — not a mockup: **61 Rust tests and 15 Python tests pass**, clippy is clean with zero warnings, and there's both a CLI demo and a local web dashboard that run a full tick-to-fill scenario end to end against the real engine. What it is *not* yet is a live trading system: order submission to Kalshi and Polymarket is deliberately incomplete (see [Status](#status)) until request signing and the live payload shape are verified against each venue's current API — see [Production readiness](#production-readiness) for exactly what that gap means.
...
- **Cross-venue arbitrage detector** — type in bid/ask for both venues, it calls `parallax-book::detect_arb` live and shows whether a riskless edge exists.
- **Backtest runner** — one button re-runs the full weather-update → stale-quote → fill scenario through the actual `parallax-sim::Backtest` pipeline (alpha aggregation, all three strategy engines, the risk gate, the matching engine) and renders the report: fills, filled volume, mark-to-model PnL, open positions.

It binds to `127.0.0.1` only, keeps everything against the in-memory `PaperAdapter`, and ships zero JS dependencies (plain HTML/CSS/vanilla JS, no build step, no CDN calls) — open the page offline and it still works once the server's running.
...
- `KalshiAdapter::submit` and `PolymarketAdapter::submit` build the request but stop short of the live HTTP call. Both venues' order-creation payload shapes were reconstructed from public documentation during this build, not exercised against a live endpoint — shipping unverified field names against an API that moves real money is the wrong tradeoff for a reference implementation. Market-data reads (`fetch_orderbook_raw`, `fetch_book_raw`, and their parsers) *are* real and tested against realistic fixtures.
...
- No live market-data ingestion loop, FIX client, or FPGA/kernel-bypass anything — see [§2 of the design doc](https://claude.ai/code/artifact/8922a0e2-37fa-4b76-affb-398dd1487c6e) for why that infrastructure doesn't apply to these venues the way the original brief assumed, and what's realistic instead.
